"""LCM type definitions
This file automatically generated by lcm.
DO NOT MODIFY BY HAND!!!!
"""

try:
    import cStringIO.StringIO as BytesIO
except ImportError:
    from io import BytesIO
import struct

class pycito_cmd_lcmt(object):
    __slots__ = ["pitch", "roll", "yaw", "footforce"]

    __typenames__ = ["float", "float", "float", "float"]

    __dimensions__ = [None, None, None, [4]]

    def __init__(self):
        self.pitch = 0.0
        self.roll = 0.0
        self.yaw = 0.0
        self.footforce = [ 0.0 for dim0 in range(4) ]

    def encode(self):
        buf = BytesIO()
        buf.write(pycito_cmd_lcmt._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">fff", self.pitch, self.roll, self.yaw))
        buf.write(struct.pack('>4f', *self.footforce[:4]))

    def decode(data):
        if hasattr(data, 'read'):
            buf = data
        else:
            buf = BytesIO(data)
        if buf.read(8) != pycito_cmd_lcmt._get_packed_fingerprint():
            raise ValueError("Decode error")
        return pycito_cmd_lcmt._decode_one(buf)
    decode = staticmethod(decode)

    def _decode_one(buf):
        self = pycito_cmd_lcmt()
        self.pitch, self.roll, self.yaw = struct.unpack(">fff", buf.read(12))
        self.footforce = struct.unpack('>4f', buf.read(16))
        return self
    _decode_one = staticmethod(_decode_one)

    def _get_hash_recursive(parents):
        if pycito_cmd_lcmt in parents: return 0
        tmphash = (0x7e3bbfa02fe297e0) & 0xffffffffffffffff
        tmphash  = (((tmphash<<1)&0xffffffffffffffff) + (tmphash>>63)) & 0xffffffffffffffff
        return tmphash
    _get_hash_recursive = staticmethod(_get_hash_recursive)
    _packed_fingerprint = None

    def _get_packed_fingerprint():
        if pycito_cmd_lcmt._packed_fingerprint is None:
            pycito_cmd_lcmt._packed_fingerprint = struct.pack(">Q", pycito_cmd_lcmt._get_hash_recursive([]))
        return pycito_cmd_lcmt._packed_fingerprint
    _get_packed_fingerprint = staticmethod(_get_packed_fingerprint)

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", pycito_cmd_lcmt._get_packed_fingerprint())[0]

