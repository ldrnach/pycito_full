...
simulation, simbuilder = A1DrakeSimulationBuilder.createSimulator(
    timestep, env, control
)
...
simplotter.plot(
    simbuilder.get_logs(), show=False, savename=os.path.join(target, "sim.png")
)
simplotter.save_data(
    simbuilder.get_logs(), savename=os.path.join(target, "simdata.pkl")
)
