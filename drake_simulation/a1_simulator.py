...
class A1DrakeSimulationBuilder:
    def __init__(self, timestep=0.01, urdf=URDF):
    ...
    @classmethod
    def createSimulator(cls, timestep, environment, controller_cls):
        """
        ...
        this.controller = controller_cls(this.plant, this.timestep)
        this._addController()
        this._addVisualizer()
    ...
    def _addController(self):
        ...
        self.builder.Connect(
            self.plant.get_state_output_port(), controller_sys.GetInputPort("state")
        )
        # Add the controller logger
        self.logger = LogVectorOutput(
            controller_sys.GetOutputPort("logging"), self.builder
        )
    ...
    def get_logs(self):
        ...
        return (
            self.logger.FindLog(self.diagram_context),
            self.contact_logger.to_dictionary(),
        )
    ...
    def get_simulation_data(self):
        ...
        return {
            "time": np.array(logs.sample_times()),
            "state": logs.data()[:nX, :],
...
class A1SimulationPlotter:
    ...
    def plot(self, logs, show=True, savename=None):
        ...
        self._plot_contact_forces(
            contactlogs, show=False, savename=utils.append_filename(savename, "forces")
        )
        self._plot_contact_results(
            contactlogs,
            show=show,
            savename=utils.append_filename(savename, "contactdata"),
        )
    ...
    def save_data(self, logs, savename):
        ...
        nX = self.a1.multibody.num_positions() + self.a1.multibody.num_velocities()
        x, u = logs.data()[:nX, :], logs.data()[nX:, :]
        data = {"time": t, "state": x, "control": u, "contact": contact}
