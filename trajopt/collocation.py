...
class LagrangeBasis():
    """Implementation of the scalar fundamental lagrange polynomial"""
    def __init__(self, nodes, centerindex=1):
        self.nodes = np.asarray(nodes)
        self.centerindex = centerindex
        self.weight = self._calculate_weight(nodes, centerindex)

    @staticmethod
    def _calculate_weight(nodes, centerindex=1):
...
class LagrangeInterpolant():
    def __init__(self, nodes, values):
        self.bases = [LagrangeBasis(nodes, i) for i in range(len(nodes))]
        self.values = np.asarray(values)
        self.nodes = np.asarray(nodes)
        self.__differentiation_matrix = self._calculate_differentiation_matrix()
    ...
    def _eval(self, x):
        ...
        if np.any(np.isinf(weights)):
            return self.values[np.isinf(weights)]
        total = np.sum(weights)
        scaled_total = np.sum(weights * self.values)
        return scaled_total / total
    ...
    def _derivative(self, x):
        ...
        """
        derivs = np.asarray([basis.derivative(x) for basis in self.bases])
    ...
    def differentiation_matrix(self):
        return self.__differentiation_matrix
...
class RadauCollocation(LagrangeInterpolant):
    def __init__(self, order=3, domain=[0,1]):
        ...
        nodes = special.eval_sh_jacobi(order-1, 2, 1)
        nodes = np.concatenate([nodes, np.ones((1,))], axis=0)
        self.interpolant = LagrangeInterpolant(nodes, values = np.zeros((order,)))
    ...
    @property
    def nodes(self):
        return self.interpolant.nodes

    @property
    def values(self):
        return self.interpolant.values

    @values.setter
    def values(self, vals):
        vals = np.asarray(vals)
        if vals.shape[vals.ndim-1] != self.order:
            raise ValueError(f"Expected {self.order} values. Got {vals.shape[vals.ndim-1]} instead")
        else:
            self.interpolant.values = vals
...
def runge_example():
    ...
    axs.plot(domain, eq_interp, linewidth=1.5, label="Equidistant Nodes")
    axs.plot(domain, radau_interp, linewidth=1.5, label="Radau Nodes")
    plt.show()
