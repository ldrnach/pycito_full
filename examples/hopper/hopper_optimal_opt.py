...
# Save directory
savedir = os.path.join('examples','hopper','optimal_longfoot')
os.makedirs(savedir)
...
options.useLinearComplementarityWithConstantSlack()
...
file = os.path.join('examples','hopper','feasible_longfoot','Slack_0E+00','trajoptresults.pkl')
data = utils.load(file)
trajopt.set_initial_guess(xtraj = data['state'], utraj=data['control'], ltraj=data['force'], jltraj=data['jointlimit'], straj = data['slacks'])
