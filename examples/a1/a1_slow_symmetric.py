...
def setup_foot_tracking_gait(a1, foot_ref, base_ref, duration, warmstart, options=None):
    ...
    trajopt = opttools.add_force_symmetry_cost(trajopt, weight=1e-2)
    # Update the solver options
    if options:
        trajopt.setSolverOptions(options)
    return trajopt

def a1_slow_steps(a1):
    generator = GaitGenerator(a1)
    #Starting configuration
    q0 = a1.standing_pose()
    q0, _ = a1.standing_pose_ik(base_pose = q0[:6], guess=q0)
    # Make the gait types
    gaits = [GaitType(stride_length=0.125, duration=0.25,step_height=0.06, swing_phase=0.75),
            GaitType(stride_length=0.125, duration=0.25, step_height=0.06, swing_phase=0.75)]
...
def a1_first_step_slow_optimization():
    # Get the footstep parameters
    savedir = os.path.join('examples','a1','foot_tracking_slow_symmetric','firststep')
    a1 = opttools.make_a1()
    feet, _, qtraj = a1_slow_steps(a1)
    foot, q = feet[0], qtraj[0]
    duration = 0.25
...
def a1_second_step_slow_optimization():
    # Get the footstep parameters
    savedir = os.path.join('examples','a1','foot_tracking_slow_symmetric','secondstep')
    a1 = opttools.make_a1()
    feet, _, qtraj = a1_slow_steps(a1)
    duration = 0.25
    ...
    sourcedir = os.path.join('examples','a1','foot_tracking_slow','firststep','weight_1e+03','trajoptresults.pkl')
...
def a1_full_step_slow_optimization():
    savedir = os.path.join('examples','a1','foot_tracking_slow_symmetric','fullstep')
    a1 = opttools.make_a1()
    feet, _, qtraj = a1_slow_steps(a1)
    ...
    sourcedir = os.path.join('examples','a1','foot_tracking_slow')
...
if __name__ == '__main__':
    a1_first_step_slow_optimization()
    a1_second_step_slow_optimization()
    a1_full_step_slow_optimization()