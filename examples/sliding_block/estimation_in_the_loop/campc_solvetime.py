...
def get_solvetime_statistics_median(logdata):
    times = np.array([log['solvetime'] for log in logdata])
    return np.median(times), iqr(times)
...
def do_median_solvetimes():
    ...
    for k, part in enumerate(PARTS):
        ...
        cpclogs = OptimizationLogger.load(os.path.join(SOURCE, part, CPC_LOG)).logs
        estlogs = OptimizationLogger.load(os.path.join(SOURCE, part, EST_LOG)).logs
        mean_mpc_time[k+1], std_mpc_time[k+1] = get_solvetime_statistics_median(mpclogs)
        mean_cpc_time[k+1], std_cpc_time[k+1] = get_solvetime_statistics_median(cpclogs)
        mean_est_time[k+1], std_est_time[k+1] = get_solvetime_statistics_median(estlogs)
    # Write data to csvfile
    colnames=['']
    colnames.extend(PARTS)
    write_to_file(os.path.join(SOURCE, 'median_solvetimes.csv'), [colnames, mean_mpc_time, mean_cpc_time, mean_est_time])
    write_to_file(os.path.join(SOURCE, 'iqr_solvetimes.csv'), [colnames, std_mpc_time, std_cpc_time, std_est_time])
    print(f"\tSaved CSV")

if __name__ == '__main__':
    do_median_solvetimes()